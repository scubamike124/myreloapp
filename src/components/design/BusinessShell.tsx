...
/** Standard Business Center — includes SOON shells for social/publish modules. */
const NAV: NavItem[] = [
...
const PRO_NAV: NavItem[] = [
  ...
  { key: "pro-tools", label: "Pro Tools", href: "/business-center/pro?tab=tools", icon: "chip" },
  { key: "pro-profile", label: "Business Profile", href: "/business-center/pro?tab=profile", icon: "contact" },
  { key: "pro-assist", label: "AI Assist", href: "/business-center/pro?tab=assist", icon: "brain" },
  { key: "pro-content", label: "Content Hub", href: "/business-center/pro?tab=content", icon: "layers" },
  { key: "pro-reports", label: "Reports", href: "/business-center/pro?tab=reports", icon: "chart" },
  { key: "create", label: "Create Studio", href: "/create", icon: "pen" },
  { key: "library", label: "Video Library", href: "/library", icon: "film" },
  { key: "brand", label: "Brand Kit", href: "/business-center/brand-kit", icon: "palette" },
  { key: "assets", label: "Assets", href: "/business-center/assets", icon: "folder" },
  { key: "publishing", label: "Publish queue", href: "/business-center/publishing", icon: "rocket" },
...
export default function BusinessShell({
  ...
  const items = variant === "pro" ? PRO_NAV : NAV;
      ...
      <aside className="sticky top-0 hidden h-screen w-[230px] shrink-0 flex-col border-r border-white/8 p-4 lg:flex" style={{ background: "linear-gradient(180deg,rgba(18,8,10,.9),rgba(8,4,5,.9))" }}>
        ...
        <nav className="flex flex-1 flex-col gap-1 overflow-y-auto">
          {items.map((n) => {
            ...
            const badge = variant === "pro" ? null : OVERVIEW_BADGES[n.key];
            return (
              <Link
                key={n.key + n.href}
                href={n.href}
        ...
        <div className="mt-4 rounded-2xl p-4 text-center" style={{ border: "1px solid rgba(255,70,85,.3)", background: "radial-gradient(200px 120px at 50% 0,rgba(225,29,42,.2),transparent 70%),rgba(14,6,8,.6)" }}>
          {variant === "pro" ? (
            <>
              <div className="text-sm font-semibold text-white/70">Pro desk</div>
              <div className="font-display text-lg font-bold" style={{ color: "#ff2d3f" }}>Live tools only</div>
              <div className="mt-1.5 text-xs" style={{ color: "#a99a9c" }}>Unfinished modules stay off this sidebar until they ship.</div>
            ...
            <>
              ...
              <div className="mt-1.5 text-xs" style={{ color: "#a99a9c" }}>Open the Pro workspace dashboard.</div>
              <Link href="/business-center/pro" className="mt-3 block rounded-lg py-2 text-sm font-bold text-white" style={{ background: "linear-gradient(135deg,#ff3645,#c4101c)" }}>Open Pro</Link>
            </>
          )}
        </div>
      </aside>
      ...
      <main className="relative min-w-0 flex-1">
        <div className="flex items-center justify-end gap-2 px-5 pt-3 sm:px-8">
          ...
          <BusinessMobileNav
            ...
            items={items.map((n) => ({
              ...n,
              badge: variant === "pro" ? null : OVERVIEW_BADGES[n.key] ?? null,
            }))}
          />
          <Link href="/account" className="flex items-center gap-2.5 rounded-xl px-3 py-2 transition-colors hover:border-[rgba(255,70,85,.45)]" style={{ border: "1px solid rgba(255,70,85,.2)", background: "rgba(14,6,8,.6)" }}>
            ...
            <div className="hidden leading-tight min-[380px]:block">
              ...
              <div className="text-[11px]" style={{ color: "#ff5663" }}>{variant === "pro" ? "Pro Desk" : "Business"}</div>
            </div>
