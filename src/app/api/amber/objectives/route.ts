...
import { createObjective, listObjectives, expandObjectivePlan } from "@/lib/amber-objectives";
...
export async function GET(req: Request) {
  ...
  return Response.json({ ok: true, objectives: await listObjectives(auth.q, userId) });
}
...
export async function POST(req: Request) {
  ...
  if (action === "create") {
    ...
    const result = await createObjective(auth.q, userId, goal, {
      deadline: str(body.deadline, 40) || null,
      actorEmail: auth.user.email,
    });
    return Response.json({ ok: true, ...result, objectives: await listObjectives(auth.q, userId) });
  }
  if (action === "expand") {
    const objectiveId = str(body.objectiveId, 80);
    if (!objectiveId) return Response.json({ ok: false, error: "objectiveId required" }, { status: 400 });
    const plan = await expandObjectivePlan(auth.q, userId, objectiveId, auth.user.email);
    return Response.json({ ok: true, plan, objectives: await listObjectives(auth.q, userId) });
  }
