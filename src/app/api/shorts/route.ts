...
export type ShortsSyncInput = { topic: string; url: string; count: number; platform: string; tone: string; languageCode: string };
...
/**
 * Command Center variant of 20 Shorts Generator — same generation logic as
 * the POST handler below, factored out so both call one implementation.
 */
export async function generateShortsSync(input: ShortsSyncInput): Promise<ShortsSyncResult> {
  const key = process.env.GEMINI_API_KEY;
  if (!key) throw new Error("GEMINI_API_KEY is not set on the server.");

  const topic = str(input.topic, 800);
  const rawUrl = str(input.url, 500);
  const count = Math.max(MIN_COUNT, Math.min(MAX_COUNT, input.count || 20));
  const platform = input.platform || "All of them";
  const tone = input.tone || "Punchy";
  const language = getLanguage(input.languageCode || "en");

  if (!topic && !rawUrl) throw new Error("Give a website address, or say what the shorts should be about.");
  ...
  if (!topic && !scanned) throw new Error("That page could not be read. Try another address, or describe the shorts instead.");

  return runShorts(key, { topic, siteText, scanned, count, platform, tone, language });
}

async function runShorts(
  key: string,
  ctx: { topic: string; siteText: string; scanned: boolean; count: number; platform: string; tone: string; language: ReturnType<typeof getLanguage> },
): Promise<ShortsSyncResult> {
  const { topic, siteText, scanned, count, platform, tone, language } = ctx;
  const prompt =
...
export async function POST(req: Request) {
  ...
  try {
    const result = await generateShortsSync({
      topic: str(body.prompt, 800),
      url: str(body.url, 500),
      count: Number(body.count) || 20,
      platform: str(body.platform, 30),
    ...
    const msg = e instanceof UnsafeUrlError ? e.message : e instanceof Error ? `Couldn't write the shorts: ${e.message}`.slice(0, 200) : "Couldn't write the shorts.";
    return Response.json({ error: msg }, { status: e instanceof UnsafeUrlError ? 400 : 502 });
  }
}
