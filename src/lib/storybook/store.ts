...
import { EBOOK_STATS_ROW_CAP, statsWindow, summariseEbooks, type EbookStats } from "./stats";
...
/**
 * "How many e-books were made in the last three days, and how many are ready
 * to publish?"
 *
 * The owner's question, answered from the shelf rather than from a hand-written
 * SQL console. Every rule about what counts — which products carry a book, what
 * makes one finished — lives in stats.ts, which has no database import and is
 * therefore testable; this is only the read.
 *
 * ## What it can and cannot see
 *
 * Only saved books. `/api/storybook` persists a story when the maker is signed
 * in and skips it otherwise, so a book generated by a signed-out visitor trying
 * the product exists in their browser tab and nowhere else. It is not missing
 * from this count so much as absent from the product's memory entirely.
 *
 * Returns null rather than zeroes when storage is unavailable: "no database"
 * and "no books" are different answers, and reporting the first as the second
 * would have the Command Center cheerfully announce a quiet week during an
 * outage.
 */
export async function ebookStats(days: number, now: Date = new Date()): Promise<EbookStats | null> {
  const q = await db();
  if (!q) return null;
  const window = statsWindow(days, now);
  try {
    /*
     * Every product is read, not just the book-bearing ones, and the filter is
     * applied in summariseEbooks. An `IN` list built from BOOK_PRODUCTS would
     * have to be interpolated — the tagged template takes one value per slot —
     * and the window is small enough that reading a few movie rows costs less
     * than hand-built SQL does.
     *
     * `pages` is selected because readiness is a fact about the pages, and
     * there is no JSON accessor that means the same thing in postgres and
     * sqlite. It is cheap: storeIllustrations moved the image bytes out to
     * storage before the row was written, so a page holds a URL, not a
     * megabyte of base64.
     */
    const rows = await q`
      SELECT id, title, product, pages, created_at AS "createdAt"
      FROM stories
      WHERE created_at >= ${window.since}
      ORDER BY created_at DESC
      LIMIT ${EBOOK_STATS_ROW_CAP}`;
    return summariseEbooks(
      rows.map((r) => ({
        id: text(r.id),
        title: text(r.title),
        product: text(r.product, "ebook") || "ebook",
        createdAt: isoText(r.createdAt),
        pages: parseJson<StoredPage[]>(r.pages, []),
      })),
      window,
    );
  } catch {
